...
namespace TranslateAndEval {
    ...
    public abstract class preCH : TypingSrc {
        ...
        public  static TypedCH Instantiate( TTuple ttuple , VBoxTU dataSrc ) {
            ...
            instance.GetType().GetProperty("DataSrc").SetValue( instance , dataSrc , new SObject[0] ) ;  // guess 
            return instance ;
        }
    ...
    */
    ...
    public class adapter_preCH:preCH {
        ...
        public override VBoxTU dataSrc   => _CH.DataSrc ;
    ...
    public class deferred_adapter_preCH : preCH {
        ...
        public override VBoxTU dataSrc  {  get { if (_CH == null ) _CH = getter() ; return _CH.DataSrc ; } }
    ...
    */
    ...
    public interface Column<T> : Column {
        ...
        TypedCH<T>           CHT     { get; }
        void FillFromFan<T_LHS>( Column<T_LHS> lhsCol , Func<T_LHS,T> f ) ;
    ...
    public abstract class ColumnImpl<T,VBoxT> : Column<T> where VBoxT : VBox<T> /* , new () */ {
        ...
        public void FillFromFan<T_LHS>( Column<T_LHS> lhsCol , Func<T_LHS,T> f ) {
            foreach ( var pred_box in lhsCol.boxesT ) {
                D.Assert( pred_box != null );
                _boxes.Add( CreateBox( pred_box , f ( pred_box.valueT() ) ));
            }
        }
    ...
    public class ColumnSingle<T> : ColumnImpl<T,VBoxSingle<T>> {
    ...
    public abstract class TranslationUnit
    {
        ...
        public ParserComb.NamedNode  ASTN;            // <- never really used as intended -- todo remove entirely 
        public abstract VBoxTR[] VBoxTrs{ get; }

#region translation phases
    ...
    /*
        VBoxTR is still not typed - it's the thing that calculates typing  
    */

    // todo - this thing is confusing as hell to implement 
    public abstract class VBoxTR : TranslationUnit ,TypingSrc {
        public virtual  MemberInfo  MI =>    null ;               // default for VBox-Edges that do not coincide with an edge in c# data graph 
        public abstract TypedCH     CH_in  { get  ; }
        public virtual  TypedCH     CH_out => preCH_out.CH;
        public          preCH       preCH_out ;                   // <- kind of dangerous, this relies on every implementation using this field - probably make this an abstract getter too 
        
        public override VBoxTR[]    VBoxTrs => new [] { this } ;
        public          TTuple      ttuple  => CH_out.ttuple;     
    }
